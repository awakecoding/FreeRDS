...
int freerds_client_outbound_scancode_keyboard_event(rdsModuleConnector* connector, DWORD flags, DWORD code, DWORD keyboardType)
{
	int length;
	int status;
	wStream* s;
...
int freerds_client_outbound_unicode_keyboard_event(rdsModuleConnector* connector, DWORD flags, DWORD code)
{
	int length;
	int status;
	wStream* s;
...
int freerds_client_outbound_mouse_event(rdsModuleConnector* connector, DWORD flags, DWORD x, DWORD y)
{
	int length;
	int status;
	wStream* s;
...
int freerds_client_outbound_logon_result(rdsModuleConnector* connector, RDS_MSG_LOGON_RESULT* msg)
{
	msg->type = RDS_CLIENT_LOGON_RESULT;
	return freerds_client_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}
...
rdsClientInterface* freerds_client_outbound_interface_new()
{
	...
	if (client)
	{
		...
		client->LogonResult = freerds_client_outbound_logon_result;
	}
...
int freerds_server_outbound_begin_update(rdsModuleConnector* connector, RDS_MSG_BEGIN_UPDATE* msg)
{
	msg->type = RDS_SERVER_BEGIN_UPDATE;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_end_update(rdsModuleConnector* connector, RDS_MSG_END_UPDATE* msg)
{
	msg->type = RDS_SERVER_END_UPDATE;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_beep(rdsModuleConnector* connector, RDS_MSG_BEEP* msg)
{
	msg->type = RDS_SERVER_BEEP;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_is_terminated(rdsModuleConnector* connector)
{
	return 0;
}

int freerds_server_outbound_opaque_rect(rdsModuleConnector* connector, RDS_MSG_OPAQUE_RECT* msg)
{
	msg->type = RDS_SERVER_OPAQUE_RECT;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_screen_blt(rdsModuleConnector* connector, RDS_MSG_SCREEN_BLT* msg)
{
	msg->type = RDS_SERVER_SCREEN_BLT;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_paint_rect(rdsModuleConnector* connector, RDS_MSG_PAINT_RECT* msg)
{
	msg->type = RDS_SERVER_PAINT_RECT;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_patblt(rdsModuleConnector* connector, RDS_MSG_PATBLT* msg)
{
	msg->type = RDS_SERVER_PATBLT;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_dstblt(rdsModuleConnector* connector, RDS_MSG_DSTBLT* msg)
{
	msg->type = RDS_SERVER_DSTBLT;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_set_pointer(rdsModuleConnector* connector, RDS_MSG_SET_POINTER* msg)
{
	msg->type = RDS_SERVER_SET_POINTER;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_set_system_pointer(rdsModuleConnector* connector, RDS_MSG_SET_SYSTEM_POINTER* msg)
{
	msg->type = RDS_SERVER_SET_SYSTEM_POINTER;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_set_palette(rdsModuleConnector* connector, RDS_MSG_SET_PALETTE* msg)
{
	msg->type = RDS_SERVER_SET_PALETTE;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_set_clipping_region(rdsModuleConnector* connector, RDS_MSG_SET_CLIPPING_REGION* msg)
{
	msg->type = RDS_SERVER_SET_CLIPPING_REGION;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_line_to(rdsModuleConnector* connector, RDS_MSG_LINE_TO* msg)
{
	msg->type = RDS_SERVER_LINE_TO;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_cache_glyph(rdsModuleConnector* connector, RDS_MSG_CACHE_GLYPH* msg)
{
	msg->type = RDS_SERVER_CACHE_GLYPH;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_glyph_index(rdsModuleConnector* connector, RDS_MSG_GLYPH_INDEX* msg)
{
	msg->type = RDS_SERVER_GLYPH_INDEX;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_shared_framebuffer(rdsModuleConnector* connector, RDS_MSG_SHARED_FRAMEBUFFER* msg)
{
	msg->type = RDS_SERVER_SHARED_FRAMEBUFFER;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_reset(rdsModuleConnector* connector, RDS_MSG_RESET* msg)
{
	msg->type = RDS_SERVER_RESET;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_create_offscreen_surface(rdsModuleConnector* connector, RDS_MSG_CREATE_OFFSCREEN_SURFACE* msg)
{
	msg->type = RDS_SERVER_CREATE_OFFSCREEN_SURFACE;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_switch_offscreen_surface(rdsModuleConnector* connector, RDS_MSG_SWITCH_OFFSCREEN_SURFACE* msg)
{
	msg->type = RDS_SERVER_SWITCH_OFFSCREEN_SURFACE;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_delete_offscreen_surface(rdsModuleConnector* connector, RDS_MSG_DELETE_OFFSCREEN_SURFACE* msg)
{
	msg->type = RDS_SERVER_DELETE_OFFSCREEN_SURFACE;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_paint_offscreen_surface(rdsModuleConnector* connector, RDS_MSG_PAINT_OFFSCREEN_SURFACE* msg)
{
	msg->type = RDS_SERVER_PAINT_OFFSCREEN_SURFACE;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_window_new_update(rdsModuleConnector* connector, RDS_MSG_WINDOW_NEW_UPDATE* msg)
{
	msg->type = RDS_SERVER_WINDOW_NEW_UPDATE;
	return freerds_server_outbound_write_message(connector, (RDS_MSG_COMMON*) msg);
}

int freerds_server_outbound_window_delete(rdsModuleConnector* connector, RDS_MSG_WINDOW_DELETE* msg)
{
